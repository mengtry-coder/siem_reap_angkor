            ...
            if ($model->validate()) {
                ...
                foreach ($names as $name) {
                    ...
                    if ($name->saveAs($path.'/'.'_'.$date.'_'.$name->baseName.'.'.$name->extension)) {
                        ...
                        $gallery_id = (new \yii\db\Query())
                        ->from('gallery')
                            ->max('id')+1;
                        $company_id = $_SESSION['company_id'];
                        Yii::$app->db->createCommand()->insert('gallery_image', ['file_name'=>$file_name, 'file_path'=>$file_path, 'gallery_id'=>$gallery_id, 'company_id'=>$company_id])->execute();
                    }
                }
            }
